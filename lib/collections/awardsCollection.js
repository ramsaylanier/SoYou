...
Meteor.methods({
	...
	postTweet: function(award, awardId){
		...
		var url = '/the/' + award.type + '/' + awardId;
		var status = "Hey, @" + award.user + ", someone thinks that youre the " + award.type + " Heres why: " + url;

		var encodedStatus = encodeURIComponent(status);
		var apiBase = 'https://api.twitter.com/1.1/statuses/update.json';

		var paramaterString = 	'oauth_consumer_key=' + consumerKey +
								...
								'&oauth_version=1.0' + 
								'&status=' + status
		var signatureString = 	"POST&" + encodeURIComponent(apiBase) + "&" + 
		...
		var signature = CryptoJS.HmacSHA1(signatureString, signingKey);
		// console.log('signature: ' + signature);
		signature = CryptoJS.enc.Base64.stringify(signature); 
		console.log('encryptedSignature: ' + signature);
		...
		var encodedSignature = oauthSignature.generate('POST', apiBase, parameters, consumerSecret, tokenSecret, { encodeSignature: false} );
